...
def add_leave():
    ...
    try:
        ...
        if not success or not new_leave.id:
            raise Exception("Failed to save the initial leave record to the database or get a valid ID.")
...
def log_leave_return(leave_id):
    ...
    try:
        ...
        if not success:
            raise Exception("Failed to update LeaveLog in the database.")
