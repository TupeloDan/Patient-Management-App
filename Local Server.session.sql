-- This is the final version of the sp_AddLeave stored procedure.
-- It now accepts all required fields, including StaffNurseID and ContactPhoneNumber.
...
CREATE PROCEDURE sp_AddLeave(
    ...
    IN p_leave_type VARCHAR(255), -- This will now be EGA, ECL, etc.
    IN p_is_escorted_leave BOOLEAN,
    IN p_staff_responsible_id INT,
    IN p_staff_nurse_id INT, -- NEW: Added parameter for MSE staff
...
BEGIN
    INSERT INTO LeaveLog (
        ...
        `Is Escorted`, StaffResponsibleID, StaffNurseID, LeaveDescription, MSE, Risk, 
        LeaveCondition, AWOL, HasContactInfo, SeniorNurseNotified, SeniorNurseID,
        ContactPhoneNumber
    )
    VALUES (
        p_nhi, p_patient_name, p_leave_date, p_leave_time, p_expected_return_time, p_leave_type, 
        p_is_escorted_leave, p_staff_responsible_id, p_staff_nurse_id, p_leave_description, p_mse_completed, p_risk_assessment_completed,
        p_leave_conditions_met, p_awol_aware, p_contact_aware, 
        (p_senior_nurse_id IS NOT NULL),
        p_senior_nurse_id,
