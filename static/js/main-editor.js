document.addEventListener('DOMContentLoaded', () => {
    ...
    let allPatients = [], selectedPatient = null;
    
    // --- REFINED: Combined all modal-opening logic into one function ---
    async function openAddLeaveModal() {
        ...
        try {
            ...
            // Add event listeners to newly created dynamic elements
    ...
    // --- REFINED: Builds the dynamic phone UI ---
    ...
    // --- NEW: Event handlers for dynamic inputs ---
    ...
    // --- REFINED: Validation function updated for both new features ---
    ...
    // --- REFINED: The submit button logic now handles all dynamic fields ---
    leaveModalSubmitBtn.addEventListener('click', async () => {
        ...
        // Get phone values
        const leaveType = addLeaveModal.querySelector('input[name="leave-type"]:checked').value;
    ...
    // ... (rest of the file is the same: functions for refreshing data, populating panes, handling selections, etc.)
    // Note: To keep the response concise, I've omitted the unchanged parts of the file.
    // You should integrate the new/modified functions above into your existing `main-editor.js`.
    const {
        refreshAllData,
        renderPatientList,
        populateDetailsPane,
        ...
        async function populateLeaveLists() {
            ...
            try {
                ...
                leaves.forEach(leave => {
                    ...
                    const leaveTime = leave.LeaveTime ? new Date(leave.LeaveTime).toLocaleTimeString('en-NZ', {
                        hour: '2-digit',
                        minute: '2-digit'
                    }) : '';
                    const returnTime = leave.ReturnTime ? new Date(leave.ReturnTime).toLocaleTimeString('en-NZ', {
                        hour: '2-digit',
                        minute: '2-digit'
                    }) : 'N/A';
                    item.textContent = `${leaveDate} | ${leaveTime} - ${returnTime} | ${leave.LeaveType}`;
                    if (leaveDate === today) {
                        leavesTodayList.appendChild(item);
                    } else {
                        allLeavesList.appendChild(item);
                    }
                });
            } catch (error) {
        ...
        modalSaveBtn.addEventListener('click', async () => {
            ...
            if (currentDateField === 'plan') {
                endpoint = `/api/people/${patientId}/update-plan-date`;
                body = {
                    completed_date: newDate
                };
            } else if (currentDateField === 'honos') {
                endpoint = `/api/people/${patientId}/update-honos-date`;
                body = {
                    completed_date: newDate
                };
            } else if (currentDateField === 'uds') {
                endpoint = `/api/people/${patientId}/update-uds-date`;
                body = {
                    last_test_date: newDate
                };
            }
            try {
        ...
        udsFrequencySelect.addEventListener('change', async (event) => {
            ...
            try {
                await fetch(`/api/people/${patientId}/update-uds-date`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({
                        last_test_date: lastUds
                    })
                });
                await refreshAllData(patientId);
            } catch (error) {
                console.error('Error recalculating UDS date:', error);
            }
        });
